...
for parent_order in session.query(ImageOrder).all():
    ...
    if parent_order.number_of_revisits > 1:
        for i in range(1, parent_order.number_of_revisits):
            # Calculate the new times
            new_start_time = parent_order.start_time + i * parent_order.revisit_frequency
            new_end_time = parent_order.end_time + i * parent_order.revisit_frequency
            new_delivery_time = parent_order.delivery_deadline + i * parent_order.revisit_frequency
