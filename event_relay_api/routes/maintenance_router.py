...
async def get_image_order_requests(id):
    requests = create_exposed_schedule_requests_query().filter(ScheduleRequest.order_id==id & ScheduleRequest.order_type=="maintenance")
    return [request._asdict() for request in requests.all()]

@router.post("/orders/{id}/requests/decline")
async def decline_maintenance_order_requests(id):
    session = get_db_session()
    order = session.query(MaintenanceOrder).filter_by(id=id).first()
    if not order:
        raise HTTPException(404, detail="Order does not exist.")
    requests = session.query(ScheduleRequest).filter_by(order_id=order.id, order_type="maintenance").all()
