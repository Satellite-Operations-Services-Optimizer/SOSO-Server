...
from app_config.database.mapping import GroundStationOutageOrder, SatelliteOutageOrder, Satellite, GroundStation, ScheduleRequest, Asset
...
async def get_all_outage_orders(response: Response, asset_type: str, page: int = Query(1, ge=1), per_page: int = Query(20, ge=1), all: bool = Query(False)):
    ...
    return paginated_response(query.all(), total)
