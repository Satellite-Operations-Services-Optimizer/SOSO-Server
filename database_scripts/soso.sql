...
BEGIN
	...
	PERFORM * FROM scheduled_contact
	...
	UPDATE scheduled_contact SET total_uplink_size = total_uplink_size + NEW.uplink_size
	WHERE id=NEW.uplink_contact_id;

	UPDATE scheduled_contact SET total_downlink_size = total_downlink_size + NEW.downlink_size
...
SELECT event_change.schedule_id,
	...
	event_change.asset_type,
	snapshot_time,
	(0.0,0.0,0.0)::asset_state as delta -- TODO: fill in the state_delta
...
CREATE TABLE IF NOT EXISTS state_checkpoint (
	...
	state asset_state NOT NULL DEFAULT (0.0, 0.0, 0.0),
	delta_from_prev_chkpt asset_state NOT NULL DEFAULT (0.0, 0.0, 0.0), -- cumulative delta of all the changes since the last checkpoint - basically how much has the state changed since the last checkpoint?
	peak_delta_from_prev_chkpt asset_state NOT NULL DEFAULT (0.0, 0.0, 0.0)-- peak state values since last checkpoint, offset from the state values at the last checkpoint. Used to see if we have exceeded the satellite's capacity since the last checkpoint, to know if we have to fix the schedule made there
);
...
INSERT INTO state_checkpoint (
	...
	NOW(), -- checkpoint_time
	(10.0, 20.0, 30.0)::asset_state, -- state
	(5.0, 10.0, 15.0)::asset_state, -- cumulative_change
	(2.0, 4.0, 6.0)::asset_state -- peak_cumulative_change
);
