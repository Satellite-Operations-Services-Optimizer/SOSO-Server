...
CREATE TABLE IF NOT EXISTS schedule (
	...
	name text UNIQUE GENERATED BY DEFAULT AS ('unnamed_schedule_'||id),
...
INSERT INTO schedule (id, name) VALUES (0, 'Default Schedule'); -- default schedule has id 0
...
CREATE TABLE IF NOT EXISTS system_order (
	id integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
	schedule_id integer NOT NULL GENERATED BY DEFAULT AS 0 REFERENCES schedule (id), -- default schedule has id 0
...
CREATE INDEX IF NOT EXISTS groundstation_outage_order_start_time_index ON gs_outage_order (start_time);
CREATE INDEX IF NOT EXISTS groundstation_outage_order_end_time_index ON gs_outage_order (end_time);
...
CREATE INDEX IF NOT EXISTS satellite_outage_order_start_time_index ON sat_outage_order (start_time);
CREATE INDEX IF NOT EXISTS satellite_outage_order_end_time_index ON sat_outage_order (end_time);
...
CREATE TABLE IF NOT EXISTS scheduled_event (
	id integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
	schedule_id integer NOT NULL GENERATED BY DEFAULT AS 0, -- this is the schedule we are in the process of constructing
...
SELECT schedule_id, 
	satellite_id, 
	snapshot_time, 
	sum(storage_delta) / satellite.storage_capacity as storage_util_delta, -- sum accounts for case where multiple events are scheduled for the same time that have effect on the load
