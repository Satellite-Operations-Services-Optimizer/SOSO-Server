...
CREATE TYPE schedule_request_status AS ENUM ('received', 'processing', 'scheduled', 'sent_to_gs', 'rejected');
...
-- The tables below don't need to be tables, they can just be views, as shown in the comments above.
-- However, sqlalchemy is refusing to automap my views :( so we'll redefine them as tables here, but when the problem is fixed, we'll make them views
-- we will not create indexes for these tables cuz they will be deleted anyways soon in favor of views
CREATE TABLE IF NOT EXISTS scheduled_imaging (
	...
	request_id integer NOT NULL REFERENCES schedule_request(id),
	uplink_contact_id integer NOT NULL REFERENCES scheduled_contact (id),
	downlink_contact_id integer DEFAULT NULL REFERENCES scheduled_contact (id),
	-- the fields below are autogenerated by the database. don't worry about them.
